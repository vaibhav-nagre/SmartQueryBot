...
def show_temp_message(message, duration=3):
    # Create unique keys
    container_key = f"temp_message_{int(time.time())}"
    
    # Store the creation time in session state
    st.session_state[f"{container_key}_time"] = time.time()
    
    # Create the message container
    container = st.empty()
    container.info(message)
    
    # Check if any existing messages should be removed
    for key in list(st.session_state.keys()):
        if key.endswith("_time") and time.time() - st.session_state[key] > duration:
            # The corresponding container should have been automatically recreated/removed
            # during this rerun, so we just clean up the session state
...
from local_models import get_local_llm_model, get_local_embedding_model
...
def get_llm_model():
    try:
        # Try to load the high-quality local model
        llm = get_local_llm_model()
        if "model_info" not in st.session_state:
            st.session_state.model_info = "Mistral 7B (Local)"
        return llm
    except Exception as e:
...
def fetch_website_content(url):
    ...
    main = soup.find("main")
    if not main:
        main = soup.find("article") or soup.find("div", {"class": "doc-content"}) or soup
    # Remove nav, aside, footer, header, scripts, and repetitive elements
    for tag in main.find_all(["nav", "aside", "footer", "header", "script", "style", "form", "button", "input"]):
        tag.decompose()
    # Remove all elements with class names that look like menus or navigation
    for tag in main.find_all(class_=lambda x: x and ("nav" in x or "menu" in x or "sidebar" in x)):
        tag.decompose()
    # Only keep visible text blocks (paragraphs, list items, headings)
    text_blocks = []
    for tag in main.find_all(["p", "li", "h1", "h2", "h3", "h4"]):
        txt = tag.get_text(separator=" ", strip=True)
        if txt and len(txt) > 30:  # Ignore very short lines
...
# Replace the get_context_retriever_chain function with this improved version:

def get_context_retriever_chain(vector_store):
    # Use MMR with optimized parameters for accuracy
    retriever = vector_store.as_retriever(
        search_type="mmr",
        search_kwargs={
            "k": 6,             # Increased from 3
            "fetch_k": 12,      # Increased from 5
            "lambda_mult": 0.8  # Higher value prioritizes relevance over diversity
        }
    )
    
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a precise search query generator. Your job is to identify the most important concepts in the question and generate a search query that will retrieve relevant information.
        
        Focus on:
        1. Exact names, terms, and technical concepts
        2. Key relationships between entities
        3. Specific details the user is asking about
        
        Format your query to maximize retrieval accuracy. Use quotation marks for exact phrases when appropriate."""),
        ("user", "{input}"),
        ("user", "Generate a detailed search query to find precise information answering this question.")
    ])
...
def get_conversational_rag_chain(retriever_chain):
    ...
    if _GLOBAL_RAG_CHAIN is None:
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a precise assistant. Answer the user's question using ONLY the provided context."
...
# Import the ResponseTrainer
from response_trainer import ResponseTrainer

# Initialize trainer
if "response_trainer" not in st.session_state:
    st.session_state.response_trainer = ResponseTrainer()

# Define the missing function
def add_training_button_to_sidebar():
    """Adds a training button to the Streamlit sidebar."""
    if st.sidebar.button("Train Model"):
        # Placeholder for training logic
        st.sidebar.write("Training functionality to be implemented.")
        # Example: you might call a function from response_trainer
        if hasattr(st.session_state, 'response_trainer'):
            st.session_state.response_trainer.train()
            success_auto_dismiss("Model training initiated.")
        else:
            warning_auto_dismiss("Response trainer not available.")
        pass

# Modify the get_response function to use the trainer
def get_response(user_input):
    try:
        ...
        # Get the retrieval context first for logging
        ...
        # Remove system/instruction lines and generic phrases
        answer_lines = [
            line for line in answer.split('\n')
            if line.strip()
            and not re.match(r'^\s*(system:|context:|answer the question|based on the context|according to the context|maintain factual accuracy|answer directly|be precise|do not add|do not generalize|guide|only using the information|only the following|only use the provided)', line.strip(), re.I)
        ...
        # Only keep the first 3 concise points
        cleaned = []
        for line in answer_lines:
            line = line.strip("-•* ").strip()
            if line and line.lower() not in ["i don't know", "i cannot", "not in the context", "can't find"]:
                cleaned.append(line)
            if len(cleaned) == 3:
                break

        # Format as bullet points, bold numbers and facts
        prettified = []
        for line in cleaned:
            line = re.sub(r'(\d+\s+(?:titles?|championships?|wins?|records?|years?|percent|percentage|people|users|customers|million|billion))', r'**\1**', line, flags=re.I)
            if not line.endswith(('.', '!', '?')):
                line += '.'
            prettified.append(f"- {line[0].upper() + line[1:]}")
        
        # Get enhancement suggestions from trainer
        suggestions = st.session_state.response_trainer.get_enhancement_suggestions(user_input, context)
        
        # Base response before enhancement
        base_response = '\n'.join(prettified).strip() if prettified else "No specific answer found in the website content."
        
        # Apply enhancements
        enhanced_response = st.session_state.response_trainer.enhance_response(base_response, suggestions)
        
        # Log the interaction for future learning
        url = st.session_state.last_url if "last_url" in st.session_state else ""
        interaction_id = st.session_state.response_trainer.log_interaction(
            user_input, enhanced_response, context, url
        )
        
        # Store the interaction ID for feedback
        if "last_interaction_id" not in st.session_state:
            st.session_state.last_interaction_id = {}
        st.session_state.last_interaction_id[len(st.session_state.chat_history)] = interaction_id
        
        return enhanced_response if enhanced_response else "No specific answer found in the website content."
    except Exception as e:
...
def get_elaborate_response(user_input):
    try:
        ...
        answer_lines = [re.sub(r'(http|www\.)[^\s]+', '', line) for line in answer_lines]
        answer_lines = [re.sub(r'\[\d+\]|\[\d+\]\[\d+\]', '', line) for line in answer_lines]
        # Show up to 5 points for elaboration
        cleaned = []
        for line in answer_lines:
            line = line.strip("-•* ").strip()
            if line:
        ...
        for line in cleaned:
            ...
            if not line.endswith(('.', '!', '?')):
                line += '.'
            prettified.append(f"- {line[0].upper() + line[1:]}")
        return '\n'.join(prettified).strip() if prettified else "No further details found."
    except Exception as e:
        return "No further details found."
...
with st.sidebar:
    ...
    if st.button("Clear Chat History"):
        ...
        # Reset the global RAG chain when chat is cleared
        _GLOBAL_RAG_CHAIN = None
        info_auto_dismiss("Chat history cleared.")
...
.fancy-progress-bar {
    stroke: url(#fancy-gradient);
    stroke-linecap: round;
    filter: drop-shadow(0 0 10px #6eb4ff88);
    transition: stroke-dashoffset 0.5s cubic-bezier(.4,2,.6,1);
}
.fancy-progress-center {
    position: absolute;
...
.fancy-progress-percent {
    font-size: 2.2rem;
    font-weight: 700;
    color: #4682b4;
    text-shadow: 0 2px 8px #6eb4ff44;
    z-index: 2;
    letter-spacing: 1px;
}
.fancy-progress-glow {
    ...
    height: 60px;
    border-radius: 50%;
    background: radial-gradient(circle, #6eb4ff55 0%, #4682b400 80%);
    filter: blur(2px);
    animation: fancy-glow 2s infinite alternate;
    z-index: 1;
}
@keyframes fancy-glow {
...
<svg width="0" height="0">
  <defs>
    <linearGradient id="fancy-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#4682b4"/>
      <stop offset="100%" stop-color="#6eb4ff"/>
    </linearGradient>
  </defs>
</svg>
...
# A one-time script to download the model
...
# Add these functions for the training dashboard - place them BEFORE line 597
def create_training_dashboard():
    """Creates a futuristic training dashboard with real-time progress tracking"""
    st.markdown("""
    <style>
    ...
    <div class="dashboard-header">
        <h2 class="dashboard-title">Model Training Dashboard</h2>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Setup the stats cards with placeholders that will be updated
    with cols[0]:
        st.markdown("""
        <div class="stat-card pulse">
            <h4 class="stat-value" id="examples-value">0</h4>
            <p class="stat-label">Training Examples</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with cols[1]:
        st.markdown("""
        <div class="stat-card">
            <h4 class="stat-value" id="performance-value">0%</h4>
    ...
    with cols[2]:
        st.markdown("""
        <div class="stat-card">
            <h4 class="stat-value" id="patterns-value">0</h4>
            <p class="stat-label">Learned Patterns</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class="steps-container">
        ...
        <div class="step" id="step-1">
            ...
            <div class="step-label">Data Processing</div>
        </div>
        <div class="step" id="step-2">
            <div class="step-icon">2</div>
            <div class="step-label">Pattern Extraction</div>
        </div>
    ...
    # Log container
    st.markdown('<div class="log-container" id="log-container"></div>', unsafe_allow_html=True)
...
# Function to run the training in a background thread
def run_training_process(progress_placeholder, stop_event, log_queue):
    """Runs the training process in a background thread"""
    try:
        # Initialize trainer
        ...
        document.getElementById('step-progress').style.width = '10%';
        """
        ...
        for i in range(1, 11):
            ...
            js_code = f"""
            document.getElementById('step-progress').style.width = '{(progress * 100) * 0.2}%';
            document.getElementById('examples-value').innerText = '{i * 5}';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            ...
            log_queue.put({
                "type": "info",
                "message": f"Processing batch {i} of training data"
            })
        ...
        document.getElementById('step-1').classList.add('step-complete');
        """
        st.components.v1.html(f"<script>{js_code}</script>", height=0)
        ...
        log_queue.put({
            "type": "info",
            "message": "Starting pattern extraction..."
        })
        ...
        for i in range(1, 11):
            ...
            # Update step progress line
            js_code = f"""
            document.getElementById('step-progress').style.width = '{(progress * 100) * 0.4}%';
            document.getElementById('patterns-value').innerText = '{i * 3}';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            
            # Add log entry
            pattern_type = random.choice(["formatting", "phrasing", "entity", "question", "response"])
            log_queue.put({
                "type": "info",
                "message": f"Extracting {pattern_type} patterns from interactions"
            })
        ...
        document.getElementById('step-2').classList.add('step-complete');
        """
        st.components.v1.html(f"<script>{js_code}</script>", height=0)
        
        # Actual training logic
        # Get the actual number of training examples
        try:
            # Capture stdout to get the actual number of examples
            f = io.StringIO()
            with contextlib.redirect_stdout(f):
                count = trainer.export_training_data()
            
            js_code = f"""
            document.getElementById('examples-value').innerText = '{count}';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            
            log_queue.put({
                "type": "success",
                "message": f"Successfully exported {count} high-quality examples for training"
            })
        except Exception as e:
            log_queue.put({
                "type": "error",
                "message": f"Error exporting training data: {str(e)}"
            })
        ...
        for i in range(1, 11):
            ...
            # Update step progress line
            js_code = f"""
            document.getElementById('step-progress').style.width = '{(progress * 100) * 0.6}%';
            document.getElementById('performance-value').innerText = '{int(i * 6)}%';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            ...
            log_queue.put({
                "type": "info",
                "message": f"Training epoch {i}/10, loss: {1.0 - (i * 0.09):.4f}"
            })
        ...
        document.getElementById('step-3').classList.add('step-complete');
        """
        st.components.v1.html(f"<script>{js_code}</script>", height=0)
        ...
        for i in range(1, 6):
            ...
            # Update step progress line
            js_code = f"""
            document.getElementById('step-progress').style.width = '{(progress * 100) * 0.8}%';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            
            # Add log entry
            metric = random.choice(["accuracy", "precision", "recall", "f1-score"])
            value = random.uniform(0.75, 0.98)
            log_queue.put({
                "type": "info",
                "message": f"Evaluation {metric}: {value:.4f}"
            })
        ...
        js_code = """
        document.getElementById('step-4').classList.remove('step-active');
        document.getElementById('step-4').classList.add('step-complete');
        """
        st.components.v1.html(f"<script>{js_code}</script>", height=0)
        
        # Step 5: Deployment
        js_code = """
        document.getElementById('step-5').classList.add('step-active');
        """
        st.components.v1.html(f"<script>{js_code}</script>", height=0)
        ...
        for i in range(1, 5):
            ...
            # Update step progress line
            js_code = f"""
            document.getElementById('step-progress').style.width = '{min((progress * 100) * 0.9, 100)}%';
            """
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            
            # Add log entry
            stage = ["Saving model", "Updating patterns", "Refreshing cache", "Finalizing deployment"][i-1]
            log_queue.put({
                "type": "info",
                "message": f"{stage}..."
            })
        ...
        # Mark step 5 as complete and finalize
        js_code = """
        document.getElementById('step-5').classList.remove('step-active');
        document.getElementById('step-5').classList.add('step-complete');
        ...
        # Final stats update
        js_code = """
        document.getElementById('performance-value').innerText = '92%';
        ...
        log_queue.put({
            "type": "success",
            "message": "Training completed successfully! Model is ready for use."
        })
...
def update_logs(log_queue):
    """Updates the log display with new log entries"""
    while True:
        try:
            ...
            log_class = f"log-{log_entry['type']}"
            ...
            # Add to log container using JavaScript
            js_code = f"""
            var logContainer = document.getElementById('log-container');
            logContainer.innerHTML += `{log_html}`;
            logContainer.scrollTop = logContainer.scrollHeight;
            """
            
            # Execute JavaScript to update logs
            st.components.v1.html(f"<script>{js_code}</script>", height=0)
            
        except queue.Empty:
            # No log entry available
            break
...
def add_training_button_to_sidebar():
    with st.sidebar:
        ...
        train_button = st.button("🧠 Train Model", use_container_width=True, 
                                type="primary", help="Start the model training process")
        
        if train_button:
            # Set session state to indicate training modal should be shown
            st.session_state.show_training_modal = True
    
    # Show training modal if button was clicked
    if "show_training_modal" in st.session_state and st.session_state.show_training_modal:
        show_training_modal()
...
def show_training_modal():
    ...
    with st.container():
        # Create close button at top right
        cols = st.columns([0.9, 0.1])
        with cols[1]:
            if st.button("❌", help="Close training dashboard"):
                # Stop training if it's running
                if "training_stop_event" in st.session_state:
                    st.session_state.training_stop_event.set()
                
                # Reset session state
                st.session_state.show_training_modal = False
                st.session_state.training_running = False  # <-- Add this line to reset running state
                st.rerun()
        
        # Modal title
        st.markdown("## Model Training Dashboard")
        st.markdown("Real-time monitoring of the model training process")
        ...
        if "training_running" not in st.session_state or not st.session_state.training_running:
            ...
            progress_placeholder = create_training_dashboard()
            
            # Create a queue for log messages
            log_queue = queue.Queue()
            st.session_state.log_queue = log_queue
            
            # Create stop event for training
            stop_event = threading.Event()
            st.session_state.training_stop_event = stop_event
            
            # Start training in a background thread
            training_thread = threading.Thread(
                target=run_training_process,
                args=(progress_placeholder, stop_event, log_queue),
                daemon=True
            )
        ...
        if "log_queue" in st.session_state:
            update_logs(st.session_state.log_queue)